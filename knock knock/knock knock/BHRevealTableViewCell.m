...
- (void)layoutSubviews {
    ...
    if (self.rightView) {
        ...
        frame.origin = CGPointMake(self.bounds.size.width - self.rightView.bounds.size.width, 0);
...
- (void)didPan:(UIPanGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        if (CGRectGetMaxX(self.contentView.frame) <= CGRectGetMinX(_rightView.frame)) {
